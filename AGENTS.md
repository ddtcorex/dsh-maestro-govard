...
- `src/host/deploy-tool.ts` — read-only deploy inspection: `govard_deploy_plan` (the `kind: "plan"` document) and `govard_deploy_check`, both scoped by `projectPath` (default: configured root, then session cwd). The mutating deploy subcommands (`deploy`, `rollback`, `sandbox *`) are deliberately not exposed; a tool that changes a target needs its own confirmation protocol first.
